<script lang="ts">
  ...
  export let downloadProgressColor: string = "#e4e4e7";
  ...
  progress.subscribe((value) => {
    if (rangeRef && thumbRef) {
      ...
      const centerThumb = (thumbWidth / 100) * value * -1;
      const centerProgressBar =
        thumbWidth + (rangeWidth / 100) * value - (thumbWidth / 100) * value;
      position = value;
...
>
  ...
  />
  <span
    class="headless-player-download"
    style={`background-color: ${downloadProgressColor}; width: ${downloadProgress}%`}
  />
  <input
    type="range"
